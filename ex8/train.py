...
class FlatFolderDataset(Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        self.paths = glob.glob(str(Path(root) / "*.jpg"))
...
class Trainer:
    ...
    def train(self) -> None:
        ...
        for epoch in range(c.epochs):
            ...
            for i, batch in enumerate(prog_bar):
                ...
                prog_bar.set_description(f'Validation loss: {loss_f:.2f}')
    ...
    def _create_model(self) -> None:
        c = self._config
        self._model = ImageTransformerModel()
        if torch.cuda.device_count() > 1:
            print(f'Using {torch.cuda.device_count()} GPUs')
            self._model = torch.nn.DataParallel(self._model)

        self._model = self._model.train().to(self._device)
    ...
    def _create_data_loader(self) -> None:
        ...
        self._transform = transforms.Compose([
            transforms.Resize(c.input_images_dim, interpolation=Image.Resampling.LANCZOS),
            transforms.RandomCrop(c.input_images_dim),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor()
        ])

        train_set_path = Path(c.dataset_path) / 'train2014'
        validation_set_path = Path(c.dataset_path) / 'val2014'
        #dataset = DatasetFolder(train_set_path, self._transform)
    ...
    def _style_loss(self, features_input: List[Tensor]) -> Tensor:
        ...
        for i in range(num_features):
            if weights[i] > 0:
                ...
                gram_target = self._style_gram_matrices[i].to(features_input[i].device)
    ...
    def _save_snapshot(self, step: int) -> None:
        ...
        if isinstance(self._model, torch.nn.DataParallel):
            torch.save(self._model.module.state_dict(), output_path)
        else:
